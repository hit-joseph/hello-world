...
# 标记
MARK = u"*" * 20
chinese_to_foreign_file = ur"/users4/xyan/translation/lexical_chinese_to_foreign.json"
...
chinese_to_foreign_dict = load(chinese_to_foreign_file)


def same_trans_num(a, b):
    list_a = chinese_to_foreign_dict[a]
    list_b = chinese_to_foreign_dict[b]
...
def extend_torch(a, b):
    results = torch.cat((a, b), 1).cuda()
    return results
...
def load_data(data_path, model, flag):
    ...
    for i, j in results:
        try:
            ...
            a = a.view(1, 100)
            b = b.view(1, 100)
            c = torch.ones((1, 1)) * same_trans_num(i, j)
            temp = torch.cat((a, b, c), 1).cuda()
            # print type(final)
            # print type(temp)
